...
/**
 * Created by Lyfebloc on 2/10/2018.
 * Lyfebloc
 */
...
public class OpenSeaService
{
    ...
    private Request buildRequest(long networkId, String api)
    {
        ...
        if (!TextUtils.isEmpty(apiKey)
                && !apiKey.equals("...")
                && com.lyfebloc.app.repository.EthereumNetworkBase.hasRealValue(networkId)
        )
        {
    ...
    public void resetOffsetRead(List<Long> networkFilter)
    {
        ...
        {
            if (com.lyfebloc.app.repository.EthereumNetworkBase.hasOpenseaAPI(networkId))
            {
    ...
    {
        if (!com.lyfebloc.app.repository.EthereumNetworkBase.hasOpenseaAPI(token.tokenInfo.chainId))
        {
            return Single.fromCallable(() -> "");
        }
        else
